...
def get_session_history(session_id: str) -> dict:
    """Returns the persisted conversation for a session so the frontend can
    restore it on page load after a browser refresh or a server restart.
    Reuses session_memory.get(), which is already PostgreSQL-backed — no
    changes to session_memory.py were needed for this."""
    session = session_memory.get(session_id)
    return {
        "session_id": session_id,
        "history": session.history,
        "active_agent": session.active_agent,
        "client_profile": session.profile.model_dump(),
    }
