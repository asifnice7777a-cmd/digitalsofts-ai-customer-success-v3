...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    pinned_agent = state.get("active_agent")
    in_meeting_flow = pinned_agent == "meeting" and not any(k in text for k in CANCEL_KEYWORDS)
    # Once the Sales Agent determines an offer needs manager approval, the
    # NEXT message is force-routed straight to human_handoff, the same way
    # an in-progress meeting flow force-routes to "meeting" — regardless of
    # what keywords (if any) that next message happens to contain.
    force_handoff = pinned_agent == "human_handoff"
...
def make_agent_node(name: str):
    def node(state: AgentState) -> AgentState:
        ...
        if name == "meeting":
            ...
            # Sales may mutate profile in place (negotiation fields). Sync
            # it back, and if the pricing rules determined this offer needs
            # manager approval, pin the NEXT turn straight to human_handoff.
            state["profile"] = profile.model_dump()
            state["active_agent"] = (
                "human_handoff" if getattr(profile, "negotiation_status", None) == "escalation_needed" else None
            )
        else:
